...
export function Settings() {
  const { theme, setTheme, logo, setLogo, setSidebarOpen, appName, appCaption, backgroundImage, setBackgroundImage } = useStore();
  ...
  const removeBg = () => setBackgroundImage('');
    ...
    <div className="page active">
      ...
      <div className="grid-2">
        ...
        <div className="card">
          ...
          <h3 className="mb-4 mt-6">Latar Belakang (Background)</h3>
          <p className="text-sm text-slate mb-4">Background kustom untuk aplikasi.</p>
          <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
            <div style={{ width: 120, height: 80, borderRadius: 12, border: '2px dashed #cbd5e1', display: 'grid', placeItems: 'center', overflow: 'hidden', background: '#f8fafc' }}>
              {backgroundImage ? <img src={backgroundImage} alt="Background" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 24, color: '#cbd5e1' }}>🖼️</span>}
            </div>
            <div>
              <input type="file" accept="image/*" ref={bgInputRef} style={{ display: 'none' }} onChange={handleBgUpload} />
              <button className="btn-primary mb-2" onClick={() => bgInputRef.current?.click()}>Upload Background</button>
              {backgroundImage && <div style={{ marginTop: 8 }}><button className="btn-ghost btn-rose" onClick={removeBg}>Hapus Background</button></div>}
              <div className="text-xs text-muted mt-2">Format: JPG, PNG, SVG (Max 2MB)</div>
            </div>
          </div>
