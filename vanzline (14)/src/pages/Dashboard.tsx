          ...
          {materials.length === 0 ? <div className="empty">Belum ada material request</div> : (
            (() => {
              const grouped = materials.reduce((acc: any, m: any) => {
                if (!acc[m.projectId]) {
                  acc[m.projectId] = { projectId: m.projectId, projectName: m.projectName || 'Legacy Project', total: 0, received: 0 };
                }
                acc[m.projectId].total += 1;
                if (m.status === 'diterima') {
                  acc[m.projectId].received += 1;
                }
                return acc;
              }, {});
              const projectMatReqs = Object.values(grouped).slice(0, 4) as any[];

              return (
                <ul style={{display:'flex',flexDirection:'column',gap:8}}>
                  {projectMatReqs.map((p: any, i) => {
                    const percent = Math.round((p.received / p.total) * 100) || 0;
                    const isDone = percent === 100;
                    return (
                      <li key={i} style={{display:'flex',gap:10,alignItems:'center',padding:8,background:'rgba(241,245,249,0.5)',borderRadius:12}}>
                        <div style={{width:36,height:36,borderRadius:10,display:'grid',placeItems:'center',fontSize:18,background:'#fff',border:'1px solid #e2e8f0'}}>
                          📦
                        </div>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:12,fontWeight:700,color:'#334155',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{p.projectName}</div>
                          <div style={{display:'flex',alignItems:'center',gap:6,marginTop:4}}>
                            <div style={{height:4,flex:1,background:'#e2e8f0',borderRadius:2,overflow:'hidden'}}>
                              <div style={{width:`${percent}%`,height:'100%',background:isDone?'#10b981':'var(--v-from)'}}></div>
                            </div>
                            <span style={{fontSize:10,fontWeight:700,color:isDone?'#10b981':'#64748b'}}>{percent}%</span>
                          </div>
                        </div>
                        <span className={`pill ${isDone ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
